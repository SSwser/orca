...
export class AgentHookServer {
  ...
  // Why: directory that holds the runtime state (endpoint.json, runtime.json).
  // Defaults to global agent-hooks dir from runtime-paths. Can be overridden
  // for tests via start()'s `runtimeDir` option.
  ...
  stop(): void {
    ...
    // Why: intentionally do NOT delete the endpoint/runtime files on stop(). A
    // stale file points at a dead port, which matches the fail-open policy (hook
    // POSTs silently fail → same as pre-file behavior). Attempting to unlink
    // introduces a TOCTOU race: a concurrent Orca instance could rewrite the
    // files between our check and unlink. The next successful start() overwrites
    // atomically; tmp-file sweep inside writeEndpointFile() handles orphans.
  ...
  // Why: mirrors endpointFilePath getter. Returns path to runtime.json.
  get metadataFilePath(): string | null {
    return this.metadataFilePathCache
  }

  // Why: exposes port + token for tests. Previously these were read from
  // buildPtyEnv(); now that method returns {} (no env transport), tests need a
  // dedicated accessor.
  getEndpointCoordinates(): { port: number; token: string } {

...
`src/shared/protocol-version.ts` says when to bump `RUNTIME_PROTOCOL_VERSION`. This page covers the changes that do _not_ bump it and are therefore easy to get wrong.
...
The frame shape can be untouched and the skew still real, because clients react to frame _content_. PR #12641 is the worked example: the host stopped synthesizing a finished agent status, and clients running older code saw different content in an identical frame.
...
Worker containment recovery changes durable lifecycle, immutable recovery disposition, Delivery resolution, workspace fencing, capacity accounting, logical Task finalization, and optional successor-generation effects together. Optional JSON fields alone are not sufficient admission: a peer missing any part could ordinary-ACK unresolved ownership, force a successor for an accepted archive, or reuse the fenced workspace.

Advertise one capability only when the runtime implements the complete contract in [`worker-terminal-lifecycle.md`](./worker-terminal-lifecycle.md): both `accept_archived_result` and `retry_with_successor`, permanent containment Delivery resolution, disposition-aware Task finalization, and durable exact successor operations. The Run home must verify that capability before accepting recovery or writing any containment state. The disposition is required semantic input, not an optional field that an older peer may ignore. Unknown or absent dispositions fail closed before mutation.

Retry additionally requires the execution host and workspace provider to advertise exact isolated-generation and durable operation readback. Archive acceptance performs no workspace or process effect, but paired, federated, and SSH recovery remain unsupported in this change unless the full home/host contract is negotiated. Do not translate either disposition into worker release, ordinary worker start, PID adoption, or a compatibility execution route.

The common Worker execution-resource lifecycle does not make resource kinds wire-compatible by itself. The execution host must negotiate the complete adapter authority contract for the exact kind: identity, owner generation, `live` / `unverifiable` / `exited` inspection, stable stop/release operation, archive receipt, and exact readback. Terminal is the first production adapter. A future Agent Session adapter needs its own complete negotiated authority; peers must not infer support from a generic resource row or fall back to Terminal, provider history, or local process inspection.

Internal lifecycle convergence is a direct cut to one schema and runtime route. Mixed-version peers remain safe by rejecting unsupported operations before any durable or external effect. That fail-closed admission is not a legacy execution path, dual write, or compatibility adapter.

The native Windows daemon protocol gate for host-crash Job containment is a local execution-host capability, not a replacement for runtime wire admission. A non-Windows peer must not advertise or infer that Windows guarantee, while an older Windows daemon may continue ordinary attach and must reject only a new supervised spawn that requires crash containment.

Atomic Worker execution start is a separate execution-host capability. It covers one idempotent Agent Session create operation containing the deterministic execution identity, WriteFence, complete first-turn prompt, and payload fingerprint. The execution host must resolve the admitted provider into a structured agent-process target containing the executable, argument vector, environment patch, and expected process identity. It must provide exact restart inspection and a provider turn-start receipt bound to the resulting Session and process incarnation. A shell command, process spawn, argument delivery, Terminal output, and transport settlement do not satisfy this capability.

The structured target is an execution-host contract, not client-selected executable authority. Worker and a remote client send logical provider intent; the host validates its installed provider, account environment, launch preferences, full rendered argument budget, containment support, and exact operation before any durable or external mutation. A string command override, unknown launcher, oversized argument vector, or host that only accepts interactive shell input is unsupported. It must not be translated into a shell command, stdin write, prompt file, post-start Terminal input, or a different execution adapter.

A renderer tab/leaf projection is not capability evidence and cannot select a fallback transport. Headless and visible surfaces of the same execution must resolve the same binding. A daemon, provider, SSH host, paired runtime, or federated host without the complete atomic-start capability must reject supervised Worker start before accepting a Dispatch, claiming an operation, creating a worktree, spawning a process, or making any other durable mutation. Socket delivery settlement, Terminal `write`, SSH transport state, renderer leaf writability, and paired-runtime reachability are not substitutes for execution-start acceptance.

The capability must cover structured process create, first-turn acceptance, exact inspection, writer fencing, and release authority together. Within Agent Session execution-start identity, WriteFence is the single writer-generation carrier; the Worker execution-resource owner generation remains a separate custody fact. The provider receipt carries one provider-observed acceptance time bound to the operation and launch token; caller observation timeouts do not participate in durable identity. After runtime restart, `started` resumes observation of the same operation without submitting input, `accepted` replays the same receipt, and `conflict` or `unverifiable` fails closed. The operation cannot be cleared to admit a different Session. Until another provider or an SSH, paired, or federated execution host advertises the full contract, absence is fail-closed and is never translated into local process death.
...
A new client against an old host sees the field absent, which is why absence must read as _unknown_ and never as _not waiting_. Collapsing absent into `null` at any hop — including a convenience `?? null` in an RPC handler — makes an old or unreachable peer indistinguishable from a healthy idle worker, which is the exact failure the field exists to remove.
